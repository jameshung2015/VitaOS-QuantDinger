    ...
    <div class="main-content-full">
      ...
      <div class="main-body">
        ...
        <div class="left-panel">
          ...
          <div class="monthly-events-box">
            <div class="box-header box-header-red">
              <span class="box-title"><a-icon type="alert" /> 本月重点待发生事件</span>
            </div>
            <div class="monthly-events-list">
              <template v-if="loadingMonthlyKeyEvents">
                <div v-for="i in 4" :key="'mke-skel-'+i" class="mke-item skeleton-item">
                  <span class="skeleton-text short"></span>
                  <span class="skeleton-text"></span>
                </div>
              </template>
              <template v-else-if="monthlyKeyEvents.items && monthlyKeyEvents.items.length > 0">
                <div v-for="(evt, idx) in monthlyKeyEvents.items.slice(0, 6)" :key="evt.event_id || idx" class="mke-item">
                  <div class="mke-top">
                    <span class="mke-date">{{ evt.date || '--' }}<span v-if="evt.time"> {{ evt.time }} 中国时间</span></span>
                    <span class="mke-bias" :class="(evt.bias || 'neutral').toLowerCase()">{{ evt.bias || 'neutral' }}</span>
                  </div>
                  <div class="mke-title">{{ evt.title }}</div>
                  <div class="mke-reason">{{ evt.reason }}</div>
                </div>
              </template>
              <template v-else>
                <div class="cal-empty">暂无重点事件</div>
              </template>
            </div>
          </div>
        </div>
              ...
              <div class="wl-card-body" :class="{ 'with-cb': batchMode }">
                ...
                <div class="wl-row-task" v-if="getMonitorMeta(stock)">
                  <span class="wl-task-badge" :class="getMonitorMeta(stock).activeCount > 0 ? 'active' : 'paused'" @click.stop="toggleStockMonitor(stock)">
                    {{ getMonitorMeta(stock).activeCount > 0
                      ? ($t('aiAssetAnalysis.monitor.enabled') || 'Enabled')
                      : ($t('aiAssetAnalysis.monitor.disabled') || 'Paused') }}
                  </span>
...
import { getMarketSentiment, getMarketOverview, getMarketHeatmap, getEconomicCalendar, getMonthlyKeyEvents } from '@/api/global-market'
...
export default {
  ...
  methods: {
    ...
    buildPositionSummary () {
      ...
      monitors.forEach(m => {
        const ids = Array.isArray(m.position_ids) ? m.position_ids.map(x => Number(x)) : []
    ...
    async toggleStockMonitor (stock) {
      ...
      try {
        ...
        this.$message.success(
          shouldEnable
            ? (this.$t('aiAssetAnalysis.monitor.enabled') || 'Enabled')
            : (this.$t('aiAssetAnalysis.monitor.disabled') || 'Disabled')
        )
    ...
    async loadMarketData () {
      ...
      this.loadingMonthlyKeyEvents = true
      this.loadMonthlyKeyEvents()
    },
    ...
    async loadMonthlyKeyEvents (force = false) {
      this.loadingMonthlyKeyEvents = true
      try {
        const res = await getMonthlyKeyEvents({ days: 30, force: force ? 1 : 0 })
        if (res?.code === 1 && res?.data) {
          this.monthlyKeyEvents = {
        ...
        console.error('Load monthly key events failed:', e)
      } finally {
        this.loadingMonthlyKeyEvents = false
        this.checkAllLoaded()
      }
    },
    checkAllLoaded () {
    ...
    .monthly-events-list {
      ...
      .mke-item {
        ...
        .mke-bias {
          ...
          padding: 0 6px;
          line-height: 16px;
          &.risk_on, &.bullish { color: #166534; background: #dcfce7; }
          &.risk_off, &.bearish { color: #991b1b; background: #fee2e2; }
          &.mixed, &.neutral { color: #92400e; background: #fef3c7; }
        }
...
.ai-analysis-container.theme-dark {
  ...
  .placeholder-hero {
    ...
    .hero-badge { background: linear-gradient(135deg, var(--primary-color, #1890ff) 0%, #0d5fc6 100%); border-color: var(--primary-color, #1890ff); color: #fff; box-shadow: 0 2px 8px rgba(24,144,255,0.4); }
    .hero-title { color: #e8e8e8; }
    .hero-subtitle, .hero-hint { color: #888; }
    .hstat { background: #1a1a1c; border-color: rgba(255,255,255,0.08); box-shadow: 0 2px 6px rgba(0,0,0,0.3); }
    .hstat:hover { border-color: color-mix(in srgb, var(--primary-color, #1890ff) 45%, transparent); box-shadow: 0 6px 20px color-mix(in srgb, var(--primary-color, #1890ff) 20%, transparent); }
    .hstat-icon { background: linear-gradient(135deg, var(--primary-color, #1890ff) 0%, #0d5fc6 100%); color: #fff; box-shadow: 0 2px 6px rgba(24,144,255,0.3); }
    .hstat-val { color: #e8e8e8; }
    .hstat-label { color: #777; }
...
.hero-badge {
  ...
  color: #fff;
  background: linear-gradient(135deg, var(--primary-color, #1890ff) 0%, #0d5fc6 100%);
...
.task-item-actions {
  ...
  .ant-btn {
    ...
    &.ant-btn-primary {
      ...
      &:hover { filter: brightness(1.1); box-shadow: 0 2px 6px color-mix(in srgb, var(--primary-color, #1890ff) 25%, transparent); }
    }
    &.ant-btn-default {
      background: #f8fafc;
      border-color: #e2e8f0;
